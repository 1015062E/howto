...
## Resolving Authentication Issue with Loopback Check for Reporting Services URL

Please follow the steps below to resolve the authentication issue related to the loopback check when accessing the Reporting Services URL. First, use Method 1 on the server `ptw-sfisbi-02v`, then restart the server and check if the problem still exists. If the problem persists after implementing Method 1 and restarting the server, please implement Method 2.
...
6. In the **Value data** box, type the CNAME or the DNS alias used for the local shares on the computer, and then click **OK**. (Do not contain leading or trailing spaces! Each host name should be on a new line. Below values are samples; you need to change these values accordingly.)
    ...
    ```
7. Exit **Registry Editor**, restart the computer, and then test if the issue persists.
...
8. Restart the computer. (You must restart the server for this change to take effect.)
